""" 
This program is the Top-Down (Memoization) solution for the Rod Cut problem
...
def rod_cut_rec(
    length: int, sizes: list[int], prices: list[int], n: int, dp: list[list[int]]
) -> int:
    ...
    # Check if we have computed this solution already
    if dp[n][length] != -1:
        return dp[n][length]
    ...
    if sizes[n] <= length:
        ...
        taken = prices[n] + rod_cut_rec(length - sizes[n], sizes, prices, n, dp)
    ...
    not_taken = rod_cut_rec(length, sizes, prices, n + 1, dp)
...
def rod_cut(length: int, sizes: list[int], prices: list[int], n: int) -> int:
    dp = [[-1 for _ in range(length + 1)] for _ in range(len(sizes) + 1)]
    return rod_cut_rec(length, sizes, prices, 0, dp)
