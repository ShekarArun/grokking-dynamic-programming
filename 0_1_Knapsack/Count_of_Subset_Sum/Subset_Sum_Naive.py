""" 
This program is the naive solution for the Count of Subset Sum problem
...
The subset [1, 3] and [4] add up to 4, so the result is 2
...
def find_subset_sum_rec(arr, n, total):
    ...
    If considering any element to contribute to the total, reduce it from the total so we know how much remaining total is to be achieved
    At any point in a recursive chain, if the total is 0, then we have achieved a combination to add up to that value.
    If current value is greater than remaining total, then skip it and continue the chain until we run out of array elements
    """
